//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.7

// ignore_for_file: unused_import

import 'package:mansis_pos_api_client/model/product_dto.dart';
import 'package:test/test.dart';

// tests for ProductDto
void main() {
  final instance = ProductDtoBuilder();
  // TODO add properties to the builder and call build()

  group(ProductDto, () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // String companyId
    test('to test the property `companyId`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String categoryId
    test('to test the property `categoryId`', () async {
      // TODO
    });

    // double purchasePrice
    test('to test the property `purchasePrice`', () async {
      // TODO
    });

    // double salePrice
    test('to test the property `salePrice`', () async {
      // TODO
    });

    // String barcode
    test('to test the property `barcode`', () async {
      // TODO
    });

    // String stockCode
    test('to test the property `stockCode`', () async {
      // TODO
    });

    // int productUnitType
    test('to test the property `productUnitType`', () async {
      // TODO
    });

    // int taxType
    test('to test the property `taxType`', () async {
      // TODO
    });

    // bool stocktaking
    test('to test the property `stocktaking`', () async {
      // TODO
    });

    // String image
    test('to test the property `image`', () async {
      // TODO
    });

    // bool storeProduct
    test('to test the property `storeProduct`', () async {
      // TODO
    });

    // bool posProduct
    test('to test the property `posProduct`', () async {
      // TODO
    });

    // bool entryProduct
    test('to test the property `entryProduct`', () async {
      // TODO
    });

    // bool favoriteProduct
    test('to test the property `favoriteProduct`', () async {
      // TODO
    });

    // int sortOrder
    test('to test the property `sortOrder`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // bool main
    test('to test the property `main`', () async {
      // TODO
    });

    // String parentId
    test('to test the property `parentId`', () async {
      // TODO
    });

    // bool active
    test('to test the property `active`', () async {
      // TODO
    });


  });

}
