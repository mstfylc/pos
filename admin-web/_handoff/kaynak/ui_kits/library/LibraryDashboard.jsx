...
const LD = window.MetronicDesignSystem_a73f8f;
  ...
  return (
    <div className={"lbd-stat"+(warn?" lbd-stat--warn":"")}>
      <div className="lbd-stat__top">
        <div className="lbd-stat__ic" style={{background:bg}}><LD.Icon name={icon} size={23} color="#fff"/></div>
        {delta && <span className={"lbd-delta "+(deltaDir==='up'?'lbd-delta--up':'lbd-delta--flat')}>
          {deltaDir==='up' && <LD.Icon name="chart-line-up" size={13} color="var(--color-success)"/>}{delta}
        </span>}
      </div>
      <div>
        ...
        <div className="lbd-stat__lbl">
          {label}
          {warn && <button className="lbd-stat__warnlink" onClick={onWarn}>İncele <LD.Icon name="chevron-right" size={12} color="var(--color-accent)"/></button>}
        </div>
      </div>
    </div>
  );
...
function LibraryDashboard({ range="7d", onNavigate }){
  ...
  const { Card, CardHeader, CardBody, CardFooter, Badge, Button, IconButton } = LD;
  const toast = LD.ToastProvider ? LD.ToastProvider.useToast() : (()=>{});
  ...
  return (
    <React.Fragment>
      ...
      <div className="lbd-grid">
        <StatCard icon="price-tag" bg="#1f3864" big={d.ciro} label="Ciro" delta={d.cd} deltaDir="up"/>
        <StatCard icon="handcart" bg="#4921ea" big={d.siparis} label="Sipariş adedi" delta={d.sd} deltaDir="up"/>
        <StatCard icon="dots-square" bg="#e08a2b" big="7 ürün" label="Eşik-altı stok" warn onWarn={()=>onNavigate&&onNavigate("stock")}/>
        <StatCard icon="heart" bg="#0bc33f" big={d.uye} label="Aktif sadakat üyesi" delta={d.ud} deltaDir="up"/>
      </div>
              ...
              {LIVE_ORDERS.map((o,i)=>{
                ...
                return (
                  <div className="lbd-order" key={i}>
                    <div className="lbd-order__ic"><LD.Icon name="handcart" size={18} color="var(--color-primary)"/></div>
                    <div className="lbd-order__m">
                      <b>{"#"+o.no}</b>
                      <span style={{whiteSpace:'nowrap'}}><LD.Icon name="home" size={12} color="var(--text-placeholder)"/>{o.branch} · {o.t} önce</span>
                    </div>
